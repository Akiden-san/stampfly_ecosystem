   ...
   **コミット選択が必要な場合（`--select`）**:
   1. `git log -20 --oneline` で直近20件を番号付きで表示:
      ...
      直近20件のコミット:
       ...
       ...
      ```
   2. AskUserQuestion で番号入力を求める（options は使わず、「Other」自由入力のみ）:
      - question: 「投稿したいコミットの番号をカンマ区切りで入力してください（例: 1,3,7）」
      - options に「直近1件」「直近3件」の簡易選択肢を用意（2個まで）
      - ユーザーは「Other」で「1,3,7」のように番号を直接入力
