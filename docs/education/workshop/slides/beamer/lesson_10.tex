...
% ------------------------------------------------------------------
\begin{frame}[fragile]{Motor Control API（1/2）}
  \framesubtitle{モータ制御: デューティ設定とミキサー}
  {\small
  \begin{tabular}{lll}
    \hline
...
% ------------------------------------------------------------------
\begin{frame}[fragile]{Motor Control API（2/2）}
  \framesubtitle{Arm / Disarm: モータ出力の有効化・無効化}
  {\small
  \begin{tabular}{lll}
    \hline
...
\begin{frame}[fragile]{IMU Sensor API}
  \framesubtitle{ジャイロ・加速度センサ（BMI270）}
  {\small
  \begin{tabular}{lll}
    \hline
...
% ------------------------------------------------------------------
\begin{frame}[fragile]{環境センサ API（1/2）}
  \framesubtitle{気圧・磁気センサ}
  {\small
  \begin{tabular}{lll}
    \hline
    \textbf{関数} & \textbf{単位} & \textbf{説明} \\
...
% ------------------------------------------------------------------
\begin{frame}[fragile]{環境センサ API（2/2）}
  \framesubtitle{ToF・光学フローセンサ}
  {\small
  \begin{tabular}{lll}
    \hline
    \textbf{関数} & \textbf{単位} & \textbf{説明} \\
...
\begin{frame}[fragile]{Teleplot}
  \framesubtitle{シリアル出力をリアルタイムグラフ化}
...
void loop_400Hz(float dt) {
  ...
  {\footnotesize
    ...
    → \texttt{sf monitor} でシリアル接続 → Teleplot パネルで自動グラフ化
  }
\end{frame}
...
void loop_400Hz(float dt) {
  ...
  \vspace{0.2em}
  {\footnotesize
    \texttt{sf monitor} でシリアル接続し、Teleplot パネルで確認（前ページ参照）
  }
\end{frame}
