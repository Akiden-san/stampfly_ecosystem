...
% ------------------------------------------------------------------
\begin{frame}[fragile]{Environmental / Distance Sensors API（1/2）: 気圧・磁気}
  \resizebox{\linewidth}{!}{%
  \begin{tabular}{lll}
    \hline
    \textbf{関数} & \textbf{単位} & \textbf{説明} \\
...
% ------------------------------------------------------------------
\begin{frame}[fragile]{Environmental / Distance Sensors API（2/2）: ToF・光学フロー}
  \resizebox{\linewidth}{!}{%
  \begin{tabular}{lll}
    \hline
    \textbf{関数} & \textbf{単位} & \textbf{説明} \\
...
void loop_400Hz(float dt) {
  ...
  \vspace{0.2em}
  {\footnotesize
    \textbf{Teleplot:} VS Code 拡張で \texttt{>name:value} 形式をリアルタイムグラフ表示。\\
    \api{sf monitor} でシリアル接続し、Teleplot パネルを開く。
  }
\end{frame}
