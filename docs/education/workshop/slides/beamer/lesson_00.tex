...
\begin{frame}{エコシステム全体像 / Ecosystem Overview}
  \begin{columns}[T]
    ...
    \begin{column}{0.45\textwidth}
      \small
      \begin{itemize}
        \item \texttt{firmware/} --- 機体・送信機の組込みコード
        \item \texttt{control/} --- 制御設計（PID, MPC）
        \item \texttt{simulator/} --- 3D 物理シミュレータ
        \item \texttt{analysis/} --- データ解析（Jupyter）
        \item \texttt{tools/} --- 開発ツール（sf CLI）
        \item \texttt{protocol/} --- 通信仕様（SSOT）
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
