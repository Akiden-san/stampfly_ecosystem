...
\begin{frame}{コントローラ各部 / Controller Layout (MODE 3)}
  \begin{center}
    \resizebox{!}{0.80\textheight}{\input{../tikz/controller_annotated}}
  \end{center}
\end{frame}
...
\begin{frame}{モータ配置とミキシング / Motor Layout \& Mixing}
  \begin{columns}[T]
    \begin{column}{0.45\textwidth}
      \centering
      \resizebox{0.95\columnwidth}{!}{\input{../tikz/motor_layout}}
    \end{column}
...
\begin{frame}[fragile]{実習: 手動ミキシング (1/2) / Hands-on: Setup}
  \begin{lstlisting}[style=sfcpp, title={user\_code.cpp --- setup}]
...
\begin{frame}[fragile]{実習: 手動ミキシング (2/2) / Hands-on: Loop}
  \begin{lstlisting}[style=sfcpp, title={user\_code.cpp --- loop}]
