...
\begin{frame}[fragile]{開発フロー / Development Flow}
  \framesubtitle{ワークショップ vs ネイティブ開発}
...
\begin{frame}[fragile]{API 比較 / API Comparison}
  \framesubtitle{ws:: ワークショップ API vs ネイティブ API}
  \begin{columns}[t]
...
\begin{frame}[fragile]{プロジェクト作成 / Create Project}
  \framesubtitle{\texttt{sf app new} で vehicle と同じ環境を独自プロジェクトとして生成}
