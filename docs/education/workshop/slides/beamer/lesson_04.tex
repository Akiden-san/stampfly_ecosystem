...
\begin{frame}{NED 座標系 / NED Coordinate System}
  \begin{columns}[T]
    \begin{column}{0.45\textwidth}
      \begin{center}
        \resizebox{!}{0.72\textheight}{\input{../tikz/imu_axes}}
      \end{center}
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{itemize}
        \item \textbf{X}: 前方 (Forward)
        \item \textbf{Y}: 右方 (Right)
        \item \textbf{Z}: 下方 (Down)
      \end{itemize}

      \vspace{1em}

      \begin{block}{BMI270 IMU}
        \begin{itemize}
          \item 6 軸: 3軸ジャイロ + 3軸加速度
          \item サンプリングレート: 400\,Hz
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}{データの可視化 / Data Visualization}
  ...
  \begin{itemize}
    ...
    \item ドローンを手で傾けながらキャプチャし、ジャイロの変化をグラフで確認しよう
  \end{itemize}
\end{frame}
