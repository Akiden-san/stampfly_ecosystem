...
| 講習の形式 | (a) 子どもがコントローラで操縦する / (b) PC からプログラム（`sf blocks` の Blockly や Python API）で機体を動かし、コントローラは離陸許可や安全停止に使う / (c) 両方 | 本計画は「複数のコントローラが同時に電波を出す」場面を前提にしている。(b) で同時に電源が入るコントローラが少なければビーコン局の必要性は下がり、代わりに PC と機体の Wi-Fi 接続のチャンネル設計が主題になる。形式を早めに確定したい |
