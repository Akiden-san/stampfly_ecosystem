# M5Stack PowerHub を TDMA ビーコン専用局にする計画（10 月 4 日講習向け）
...
ビーコン無しで運用していたことになる。10 月 4 日の講習では、講師側が管理する専用の
ビーコン局を各チャンネルに置いて、この依存を断つ。
...
| 期限 | 2026-10-04（日）講習。9 月 22 日を代替案（ビーコン専用の最小ファーム）への切替判断日とする |
...
| `docs/events/<10/4 講習>/` | 機材チェックリスト（PowerHub 3 台・電池・アンテナ・ラベル）、設置手順（電源投入 → 各コントローラの SYNC 確認）、トラブル対応表（LOST が出た、二重ビーコン、チャンネル違い） |
| 筐体ラベル | 「CH1 / CH6 / CH11」 |
