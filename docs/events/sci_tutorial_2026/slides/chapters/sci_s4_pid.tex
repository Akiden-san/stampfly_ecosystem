...
\begin{frame}{内側ループ: レート制御}
  \begin{center}
    \resizebox{0.85\textwidth}{!}{\input{../../_shared/tikz/feedback_block}}
  \end{center}
...
\begin{frame}{PID 制御器}
  \begin{center}
    \resizebox{0.78\textwidth}{!}{\input{../../_shared/tikz/pid_block}}
  \end{center}
...
\begin{frame}{実測パラメータ}
  ...
  {\scriptsize 注: この $K_m$（duty$\to$トルク，N$\cdot$m）はモータの逆起電力定数
  （back-EMF constant，V/(rad/s)）とは別物（記号が同じだけ）}
...
% ------------------------------------------------------------------
\begin{frame}{無駄時間の影響}
  {\small 前の「開ループボード線図」で見た $L(s)$ に、むだ時間を加える}
...
\begin{frame}{モデルで見る実習 5 の設計}
  {\small $\zeta = 0.7$ の設計式（「2次系設計」）から求めた $K_p$ と、実習 5 で勘で置いた $K_p = 0.5$ を比べる}
  ...
  \begin{exampleblock}{読み取り}
    ...
    ここから I 項・D 項を追加する
  \end{exampleblock}
  % Web版のみ: 実習5のP制御プリセットでステップ応答を再現（収まらない場合は
  % 「PID 制御器」フレームのウィジェットのみに絞る）
  % @web: widget=pid_step axis=roll preset=lesson5 height=230
\end{frame}
...
% ------------------------------------------------------------------
\begin{frame}{PID 工学形式パラメータ\later}
  \begin{table}
  ...
  （$K_p$ は Roll/Pitch が「2次系設計」の $\zeta=0.7$ 設計値，Yaw のみ経験値。
  設計式に $K=8.0$ を当てはめると $\zeta\approx0.88$，減衰をやや強めた設定に相当する）。
  不完全微分フィルタと $\eta$ は未使用（vehicle 実装が追加する改良点）}
\end{frame}

% ------------------------------------------------------------------
\begin{frame}[fragile]{D 項とアンチワインドアップ\later}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
...
% ------------------------------------------------------------------
\begin{frame}{離散化と ARM 時リセット\later}
...
\begin{frame}{相補フィルタと STABILIZE}
  \begin{columns}[c]
    \begin{column}{0.50\textwidth}
      \centering
      \resizebox{\linewidth}{!}{\input{../../_shared/tikz/comp_filter}}
    \end{column}
    \begin{column}{0.48\textwidth}
...
\begin{frame}[fragile]{自動チューニング: sf sysid rate-fit / rate-tune}
  \begin{block}{ETFE によるプラント同定と仕様ベースのゲイン設計}\scriptsize
    チャープ（またはダブレット）励振で軸ごとに $(b, L, T)$ を推定し，
    $G_p(s) = b\,e^{-Ls}/(s(Ts+1))$ にフィットする。sf sysid fit（P 制御前提の逆算）と違い，
    実際に飛んだ PID 出力を再構成してから ETFE を取る閉ループ同定
  \end{block}
...
sf sysid rate-tune --fit fit.json --wc 25 --pm 60
  ...
  \vspace{-0.3em}
  {\scriptsize \warn{注:} 学習者コードで rate-fit/rate-tune を使うには，角速度目標の計算直後に
...
sf sils scenario simulator/sils/scenarios/workshop_acro_step.scn --target workshop
  ...
  {\small S5「実習: 自分の PID を SILS で飛ばす」も \code{workshop\_acro.scn} で同様に再現できる}
\end{frame}

% ------------------------------------------------------------------
\begin{frame}{理論とコードの対応表\later}
  {\small 目的: 同じ概念を指す変数・パラメータが，学習者コード／vehicle 実装／
  ...
  \begin{alertblock}{確認事項}\small
    \begin{itemize}\setlength{\itemsep}{1pt}
      ...
      \item[$\square$] 不完全微分・アンチワインドアップ・ARM 時リセットで
            ...
            （詳しくは持ち帰り資料「D 項とアンチワインドアップ」「離散化と ARM 時リセット」）
