...
\begin{frame}[fragile]{実習 5: レート P 制御で初飛行}
  ...
  \begin{enumerate}\footnotesize\setlength{\itemsep}{0pt}
    ...
    \item \code{user\_code.cpp} の TODO を埋める（\api{ws::gyro\_x/y/z()}，\api{ws::rc\_roll/pitch/yaw()}）
    \item \code{sf lesson build}
    \item \code{sf lesson flash}
    \item 確認: 講師が完成コード（$K_p=0.5$）で飛ばす。スティック追従とわずかな振動を見る
  \end{enumerate}

  {\footnotesize 帰宅後に自分の $K_p$ で飛ばして再現する}
\end{frame}

% ------------------------------------------------------------------
\begin{frame}[fragile]{実習 5 の振り返り: 学習者コード}
...
\begin{frame}[fragile]{実習 6: システムモデリング}
  ...
  \begin{enumerate}\footnotesize\setlength{\itemsep}{0pt}
    ...
    \item \code{Kp\_roll/pitch/yaw} を設計式で計算するコードを書く
    \item \code{sf lesson build}
    \item \code{sf lesson flash}
...
% ------------------------------------------------------------------
\begin{frame}{なぜ閉ループデータから開ループ同定ができるか\later}
  \begin{block}{$K_p$ が既知なら，閉ループデータから開ループプラントを復元できる}
    \footnotesize
    \setlength{\abovedisplayskip}{2pt}\setlength{\belowdisplayskip}{2pt}%
    レートループは P 制御で飛ばしている（実習5・7）。$K_p$ は自分で設定した
    既知の値なので，Data Stream に残る目標値と計測値から，プラントへの
    入力そのものを逆算できる:
    \[
      u(t) = K_p\bigl(\text{rate\_ref}(t) - \text{gyro}(t)\bigr), \qquad
      y(t) = \text{gyro}(t)
    \]
  ...
  {\footnotesize \code{sf sysid fit} はこれに加え，ホバリング区間の自動検出，
  短いセグメントへの分割，結果の統計処理（中央値・不確かさ）を自動で行う}
\end{frame}
...
\begin{frame}[fragile]{実習 7: システム同定（sf sysid fit）}
  ...
  \begin{enumerate}\footnotesize\setlength{\itemsep}{0pt}
    ...
    \item $K_p$ を設定し \api{ws::set\_rate\_target} を呼ぶ
    \item \code{sf lesson build}
    \item \code{sf lesson flash}
    \item 飛行しながら \code{sf log wifi -o flight.csv}
    \item \code{sf sysid fit flight.csv --kp 0.5 --plot}
  \end{enumerate}

  {\footnotesize 確認: 同定結果 $K,\tau_m$ が実習 6 の理論値と数\%の誤差で一致する。
  帰宅後に自分のフライトデータで再現する。\code{sf log wifi} は WiFi 接続が前提
...
% ------------------------------------------------------------------
\begin{frame}{工学形式パラメータ $K_p,T_i,T_d,\eta$\later}
  \begin{table}
    \centering\footnotesize
  ...
  {\footnotesize この形式（$K_p,T_i,T_d,\eta$）は\textbf{工学形式}と呼ばれ，
  vehicle 実装の \code{rate.<axis>.kp/ti/td} パラメータがそのまま使う形。
  積分・微分の「効き方」を時間の単位で直感的に調整できるのが利点}
\end{frame}

% ------------------------------------------------------------------
\begin{frame}{教科書形式との対応と実習8の解答ゲイン\later}
  \begin{block}{工学形式 $\leftrightarrow$ 教科書形式（直接ゲイン）}\footnotesize
    \[
  ...
  {\footnotesize 実習 8 の学習者コードは工学形式ではなく，教科書形式の直接ゲイン
  $(K_p,K_i,K_d)$ を使う}
...
% ------------------------------------------------------------------
\begin{frame}[fragile]{実習 8: PID 制御}
  \vspace{0.15em}
  ...
  \begin{enumerate}\footnotesize\setlength{\itemsep}{0pt}
    ...
    \item 軸ごとに I 項・D 項・アンチワインドアップを追加
    \item \code{sf lesson build}
    \item \code{sf lesson flash}
    \item 確認: 実習 5（P のみ）と比べて定常偏差とオーバーシュートの変化を見る
  \end{enumerate}

  {\footnotesize 帰宅後に自分のゲインで再現する}
\end{frame}
...
\begin{frame}{アンチワインドアップ: クランプ vs 条件付き積分\later}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{実習8: 積分クランプ}\footnotesize
        \setlength{\abovedisplayskip}{2pt}\setlength{\belowdisplayskip}{2pt}%
        \[
          \begin{array}{l}
            \texttt{I}\;{+}{=}\;e\cdot dt \\
...
\begin{frame}[fragile]{実習 9: 姿勢推定（相補フィルタ）}
  ...
  {\footnotesize 確認: 機体を手で傾け，両者が近い挙動をすることを見る。帰宅後に $\alpha$ を変えて再現する}
\end{frame}
...
\begin{frame}{周波数掃引同定の原理: チャープ励振と ETFE\later}
  ...
  {\footnotesize \code{sf sysid fit}（実習7）は P 制御前提の逆算だった。ここでは
  実際に飛んだ PID の入出力から，プラントの\textbf{周波数応答}を直接推定する。
...
\begin{frame}{一次遅れ+むだ時間モデルへの当てはめ\later}
  ...
  {\footnotesize 実習7の \code{sf sysid fit} は $K_p$ 一定・低速の飛行が前提だが，
...
% ------------------------------------------------------------------
\begin{frame}[fragile]{sf sysid rate-fit / rate-tune の使い方\later}
  \begin{block}{ETFE によるプラント同定と仕様ベースのゲイン設計}\footnotesize
    チャープ励振で軸ごとに $(b,L,T)$ を推定し $G_p(s)=b\,e^{-Ls}/(s(Ts+1))$ に
    フィットする。sf sysid fit（P 制御前提の逆算）と違い，実際に飛んだ PID 出力を
    再構成してから ETFE を取る閉ループ同定
  \end{block}
...
sf sysid rate-tune --fit fit.json --wc 25 --pm 60
  ...
  {\scriptsize \code{rate-tune} はゲイン交差周波数 $\omega_c$ と位相余裕 PM の仕様を満たす
  $K_p/T_i/T_d$ を逆算し，\code{param set rate.roll.kp ...} の形で出力する（WiFi の CLI に打てば
  飛行中も即時反映。\code{sf log wifi} と同じく WiFi 接続が前提，S2 参照）。
  \warn{注:} 学習者コードで rate-fit/rate-tune を使うには，角速度目標の計算直後に
  \api{ws::set\_rate\_target(roll,pitch,yaw)} を呼ぶ（実習 7 で必須，Data Stream の
  \code{rate\_ref\_*} に記録）}
\end{frame}
...
\begin{frame}{デモの期待結果: P と PID のステップ応答}
  ...
  % @web: video=../fallback/S4_lesson5_p_flight.mp4 caption=実習 5（P のみ）
  % @web: video=../fallback/S4_lesson8_pid_flight.mp4 caption=実習 8（PID）
  % @web: layout=replace
\end{frame}
...
sf sils scenario simulator/sils/scenarios/workshop_acro_step.scn --target workshop
  ...
  {\footnotesize S5「実習: 自分の PID を SILS で飛ばす」も \code{workshop\_acro.scn} で同様に再現できる}
\end{frame}
...
\begin{frame}{理論とコードの対応表}
  {\footnotesize 同じ概念を指す変数・パラメータが，学習者コード／vehicle 実装／
  ...
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{22mm}
    ...
    \hline
    \textbf{教科書表記} & \textbf{実習8 学習者} & \textbf{vehicle 実装} & \textbf{Data Stream} \\
    \hline
  ...
  {\footnotesize レートループの $K_p$ は，学習者コードでは duty 空間の無次元値，
  vehicle 実装では物理トルク単位。数値を単純比較できない点が
  「学習者コードと vehicle 実装の違い」を最もよく表す}
\end{frame}
...
\begin{frame}{復習パス}
  ...
  {\scriptsize 複数回に分けたハンズオンとして通しでやり直したい場合は，
  Workshop スライド（\code{docs/events/stampfly\_workshop/slides/chapters/}）の
  対応する Lesson 5〜9 章も参照できる（同じ実習の，より時間をかけた版）}
\end{frame}
...
\begin{frame}{チェックポイント}
  \begin{alertblock}{確認事項}\footnotesize
    \begin{itemize}\setlength{\itemsep}{1pt}
      ...
      \item[$\square$] 不完全微分・アンチワインドアップ・ARM 時リセットで学習者
