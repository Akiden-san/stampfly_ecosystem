...
\begin{frame}[fragile]{実習 1: 環境確認とビルド}
  ...
  \begin{block}{手順}
    \begin{enumerate}\footnotesize\setlength{\itemsep}{0pt}
      \item \code{source setup\_env.sh}（Windows は \code{setup\_env.bat}）
  ...
  \begin{exampleblock}{確認すること}
    \footnotesize 書き込み直後の起動音と、機体上の LED が点灯・点滅すること
  \end{exampleblock}
...
\begin{frame}{機体の軸定義}
  \vspace{-0.5em}
  \begin{center}
    \resizebox{0.6\textwidth}{!}{\input{../../_shared/tikz/imu_axes}}
  \end{center}
  \vspace{-0.5em}
  \begin{block}{}
    \centering\small
...
\begin{frame}{他のセンサ\later}
  ...
  {\footnotesize 各ディレクトリで単独ビルド可能: \code{idf.py set-target esp32s3 \&\& idf.py build}}
\end{frame}
