...
% ------------------------------------------------------------------
\begin{frame}[fragile]{実習 1: 環境確認とビルド}
  \vspace{0.15em}
  ...
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      ...
      \begin{exampleblock}{切替・編集・ビルド・書き込み}
        ...
        \code{sf monitor workshop}
      \end{exampleblock}
    \end{column}
    \begin{column}{0.40\textwidth}
      \centering
      \adjustbox{max width=\columnwidth, max height=0.78\textheight}{\input{../../_shared/tikz/sci_build_flash_flow}}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{機体の軸定義}
  ...
  \end{block}
  {\footnotesize StampFly では IMU の軸をそのまま機体の Body 座標系として採用している}
\end{frame}
...
\begin{frame}[fragile]{実習 2: IMU の値を見る}
  ...
  \begin{block}{手順}
    \begin{enumerate}\footnotesize\setlength{\itemsep}{1pt}
      ...
      \item \code{sf lesson edit} で \code{user\_code.cpp} を開き、次ページのコードを書く
      \item \code{sf lesson build}
      \item \code{sf lesson flash}
      \item \code{sf monitor workshop}（\code{sf lesson flash} 直後はそのまま開く）
    \end{enumerate}
  \end{block}
...
\begin{frame}[fragile]{接続手順: 機体を WiFi 網に入れる}
  \begin{block}{機体側: 初回だけ USB で設定する（以後は電源投入だけでよい）}
    \footnotesize
    \begin{enumerate}\setlength{\itemsep}{0pt}
      \item USB 接続のまま \code{sf monitor} を開き CLI に入る（終了は \code{Ctrl+]}）
      \item \code{param set wifi.mode 1}（\code{0}=STA が既定／\code{1}=SoftAP）
      \item \code{param save} してから \code{reboot}
    \end{enumerate}
  \end{block}
  ...
  {\scriptsize 疎通確認: \code{sf telemetry}（受信できれば OK，IP 指定不要）。
...
\begin{frame}{復習パス\later}
  ...
  \begin{tabular}{lll}
    ...
    \hline
    \code{sf doctor} / \code{sf build vehicle} / \code{sf flash vehicle -m} & 実習1（環境確認とビルド） & \code{docs/guides/troubleshooting.md} \\
