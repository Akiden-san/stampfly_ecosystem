...
\begin{frame}{提供: シミュレータ --- 役割の違う 3 つの実行環境}
  ...
  \begin{block}{なぜ 1 つでは足りないか}
    \footnotesize 「実機のファームをそのまま動かして検証する」「中身を読んで作り方を学ぶ」「強化学習を回す」は要求が違い、1 つの実装では両立しない。3 つは同じ物理パラメータ（\code{stampfly\_physical.yaml}、\code{sf params check} で照合）を共有する。ゲイン設計に使う線形モデル（S4）はシミュレータではなく設計の道具
  \end{block}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \begin{itemize}\scriptsize\setlength{\itemsep}{1pt}
        ...
        \item \textbf{試験の与え方}: シナリオ \code{.scn} に操縦・外乱・故障を時系列で書き、\code{.expect} で PASS / FAIL を自動判定。CI で退行を検出
