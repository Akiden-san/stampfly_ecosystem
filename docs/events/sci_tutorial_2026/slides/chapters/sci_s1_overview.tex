...
\begin{frame}{要点3つ}
  ...
  {\small 後半では、飛ばす前後で同じモデルを使って結果を確かめる仕組み（シミュレーション 3 層）にも触れる}
\end{frame}
...
\begin{frame}{設計思想5本柱}
  \begin{table}
    ...
    \begin{tabular}{lp{104mm}}
      ...
      \textbf{柱} & \textbf{内容} \\
      \hline
      集約と SSOT & ファーム・シミュレータ・解析・教材を単一リポジトリに集約。通信仕様は \code{protocol/spec/} を SSOT（Single Source of Truth、唯一の正となる情報源）とする \\
      sf CLI & ビルド・書込み・ログ取得・シミュレーション操作を統一コマンド体系で提供する（CLI: コマンドラインツール） \\
      4 階層アクセス & L0 実習用 API（\code{ws::}）から Topic API・HAL・ハードウェアまで 4 つの入口が同時に存在し、学習者は必要な深さから入れる \\
...
\begin{frame}{1機から学べる8分野}
  {\small この 5 本柱の上で、1 機から次の 8 分野を扱える}
...
\begin{frame}{深掘り: 4階層は同時に存在する\later}
  \small \code{ws::} 名前空間（L0 実習用 API）は、制御ループの起動やセンサ初期化を隠しつつ、\api{setup()} と \api{loop\_400Hz(dt)} だけで機体を動かせるようにする

  \vspace{0.5em}

  \begin{itemize}\small
    \item L0 実習用 API・L1 Topic API・L2 HAL・L3 ハードウェア直叩きの4層は、実習の進行に応じて後から現れるのではなく、ファームウェア起動時点からすべて実装済みで並行して存在する
...
\begin{frame}{シミュレーション3層}
  ...
  {\small 5 本柱の「シミュレータ統合」を掘り下げる。目的の異なる 3 層を使い分ける}
  ...
  {\small 3 層とも同じ物理パラメータを参照するため、飛ばす前の設計（層 1・層 2）と飛ばした後の答え合わせ（層 3）が同じモデルの上でつながる。合否の基準と SILS の実習は S5 で扱う}
\end{frame}
...
% 使い方と実習は S5 で扱う。
\begin{frame}[fragile]{層 3 を画面で触る: sf sils gui}
  \vspace{0.15em}
