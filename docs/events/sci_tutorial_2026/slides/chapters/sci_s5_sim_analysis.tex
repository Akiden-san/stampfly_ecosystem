...
\begin{frame}{シミュレーションの3層構造}
  \begin{center}
    \resizebox{0.95\textwidth}{!}{\input{../../_shared/tikz/sci_sim_layers}}
  \end{center}
...
sf lesson sils
  ...
  {\scriptsize 自分のコードで試す場合は\code{--solution}を付けずに切り替えるだけでよい。
...
\begin{frame}{理論とコードの対応表\later}
  ...
  \begin{table}
    ...
    \begin{tabular}{>{\raggedright\arraybackslash}p{35mm}>{\raggedright\arraybackslash}p{58mm}>{\raggedright\arraybackslash}p{35mm}}
      ...
      \hline
      層1: 設計用線形モデル       & \code{control/\allowbreak models/\allowbreak stampfly\_physical.yaml} & \code{sf sysid rate-fit} \\
      層2: 実ログ駆動オフライン再生 & \code{analysis/\allowbreak scripts/}                       & --- \\
      層3: SILS                   & \code{simulator/\allowbreak sils/}                         & \code{sf sils scenario} \\
      モデル一致の合否判定         & \code{tools/\allowbreak log\_analyzer/\allowbreak rate\_sysid.py}      & \code{sf sils sysid-gate} \\
