...
\begin{frame}{5セッション復習パス (1/2)}
  \begin{table}
    ...
    \begin{tabular}{l>{\raggedright\arraybackslash}p{40mm}>{\raggedright\arraybackslash}p{33mm}>{\raggedright\arraybackslash}p{55mm}}
      ...
      S1 & \code{sf sils gui}               & SILS の体験             & \code{simulator/\allowbreak sils/\allowbreak gui/\allowbreak README.md} \\
...
\begin{frame}{5セッション復習パス (2/2)}
  \begin{table}
    ...
    \begin{tabular}{l>{\raggedright\arraybackslash}p{40mm}>{\raggedright\arraybackslash}p{33mm}>{\raggedright\arraybackslash}p{55mm}}
      ...
      & \textbf{コマンド} & \textbf{扱う内容} & \textbf{文書} \\
      \hline
      S4 & \code{sf lesson switch sci2026:8} & PID 制御（実習8）       & 本資料 S4「PID 制御器の構成」〜「ARM 遷移時の積分リセット」 \\
      S4 & \code{sf sysid fit}              & システム同定（実習7）    & 本資料 S4「システム同定の考え方」〜「同定結果と理論値の比較」 \\
