...
\begin{frame}[fragile]{sf CLI チートシート (1/2)}
  \begin{table}
    ...
    \begin{tabular}{l>{\raggedright\arraybackslash}p{75mm}}
      ...
      \code{sf competition hover-time/score} & ホバー耐久・スコア記録 \\
      \hline
    \end{tabular}
  \end{table}
\end{frame}
...
\begin{frame}[fragile]{sf CLI チートシート (2/2)}
  \begin{table}
    ...
    \begin{tabular}{l>{\raggedright\arraybackslash}p{75mm}}
      ...
      \code{sf sysid rate-fit/rate-tune} & 閉ループ同定（ETFE + フィット）と仕様ベース PID 設計（S4） \\
...
\begin{frame}{5セッション復習パス (1/2)}
  \begin{table}
    ...
    \begin{tabular}{l>{\raggedright\arraybackslash}p{40mm}>{\raggedright\arraybackslash}p{33mm}>{\raggedright\arraybackslash}p{55mm}}
      ...
      \hline
      S1 & \code{sf sim run vpython}        & シミュレータ操縦        & \code{docs/\allowbreak commands/\allowbreak sf-sim.md} \\
      S1 & \code{sf sils gui}               & 層 3 の体験             & \code{simulator/\allowbreak sils/\allowbreak gui/\allowbreak README.md} \\
      S2 & \code{sf lesson switch sci2026:2} & IMU センサ（実習2）    & \code{firmware/\allowbreak vehicle/\allowbreak docs/\allowbreak architecture.md \S2} \\
      S2 & \code{sf log wifi/viz}           & テレメトリ取得と可視化  & \code{docs/\allowbreak guides/\allowbreak flight-log-viz.md} \\
      S3 & \code{sf lesson switch sci2026:3} & モータ制御（実習3）     & \code{firmware/\allowbreak vehicle/\allowbreak docs/\allowbreak hardware\_init.md} \\
      S3 & \code{sf lesson switch sci2026:4} & コントローラ入力（実習4） & \code{protocol/\allowbreak spec/}（ControlPacket） \\
      \hline
    \end{tabular}
  \end{table}
\end{frame}
...
\begin{frame}{5セッション復習パス (2/2)}
  \begin{table}
    ...
    \begin{tabular}{l>{\raggedright\arraybackslash}p{40mm}>{\raggedright\arraybackslash}p{33mm}>{\raggedright\arraybackslash}p{55mm}}
      ...
      & \textbf{コマンド} & \textbf{扱う内容} & \textbf{文書} \\
      \hline
      S4 & \code{sf lesson switch sci2026:8} & PID 制御（実習8）       & Workshop Lesson 8 \\
      S4 & \code{sf sysid fit}              & システム同定（実習7）    & Workshop Lesson 7 \\
      S5 & \code{sf sils gui}               & SILS ブラウザ実験環境   & \code{simulator/\allowbreak sils/\allowbreak gui/\allowbreak README.md} \\
      S5 & \code{sf log viz}                & ログ解析                & \code{docs/\allowbreak guides/\allowbreak flight-log-viz.md} \\
      \hline
    \end{tabular}
  \end{table}
  ...
  {\footnotesize Workshop スライド（\code{docs/\allowbreak events/\allowbreak stampfly\_workshop/\allowbreak slides/\allowbreak chapters/}）内の該当 Lesson 章に対応}
\end{frame}
