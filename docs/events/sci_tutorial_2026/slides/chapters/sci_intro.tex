...
\begin{frame}[fragile]{開発環境の導入（先出し）: PC 側}
  {\small 未導入の方は Session 1 を聞きながら並行して進めてほしい（Windows 主体で書く）}

  \begin{block}{方法①: GUI インストーラ「StampFly Setup」}
    \footnotesize Releases の \code{StampFlySetup\_windows-x64.exe}（macOS / Linux 版もあり）を起動し、ウィザードに従えば \code{sf} CLI 一式（ESP-IDF 込み）が入る。SmartScreen 警告は「詳細情報」$\to$「実行」\\
    \url{https://github.com/M5Fly-kanazawa/stampfly_ecosystem/releases/latest}
  \end{block}

  \begin{exampleblock}{方法②: CLI インストーラ（CMD で）}
    \begin{lstlisting}[style=sfbash, numbers=none, aboveskip=2pt, belowskip=0pt]
git clone https://github.com/M5Fly-kanazawa/stampfly_ecosystem
cd stampfly_ecosystem
install.bat           # macOS / Linux は ./install.sh
    \end{lstlisting}
  \end{exampleblock}

  {\scriptsize どちらも Git が必要（未導入なら \url{https://git-scm.com/download/win}）。導入確認（\code{setup\_env.bat} $\to$ \code{sf doctor}、macOS / Linux は \code{source setup\_env.sh}）は Session 2 の「実習1」で行う}
\end{frame}
...
\begin{frame}{必要機材と安全ルール}
  ...
  \begin{alertblock}{飛行時の安全ルール}
    \begin{itemize}\small\setlength{\itemsep}{2pt}
      \item 実機の飛行は講師が行う。参加者は機体から距離を保つ
