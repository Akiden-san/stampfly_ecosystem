...
% (setup_env -> sf doctor) stays in S2's "実習1", which is why this frame's
% closing note now points there explicitly instead of "このあと".
...
% 本フレーム末尾の注記は「このあと」ではなく明示的にS2を指す。
\begin{frame}[fragile]{開発環境の導入（先出し）: PC 側}
  {\small 未導入の方は Session 1 を聞きながら並行して。導入は CLI で行う（Windows のコマンドプロンプトで書く）}

  {\footnotesize \textbf{① 前提ツール}: Git と Python 3.12 を入れる（Windows は \code{winget}、macOS は \code{brew}、Linux は \code{apt}。コマンドはリポジトリ直下 README）。終わったら CMD を開き直す}
  \begin{exampleblock}{② 取得と導入}
    \begin{lstlisting}[style=sfbash, numbers=none, aboveskip=1pt, belowskip=0pt]
git clone https://github.com/M5Fly-kanazawa/stampfly_ecosystem.git
cd stampfly_ecosystem
install.bat
    \end{lstlisting}
    \footnotesize macOS / Linux は \code{./install.sh}。ESP-IDF を尋ねられたら 1 を選ぶ
  \end{exampleblock}
  {\footnotesize \textbf{③ 有効化と診断}: 新しい CMD を開くたびに \code{setup\_env.bat}（macOS / Linux は \code{source setup\_env.sh}）、続けて \code{sf doctor}}

  {\scriptsize GUI 版インストーラは安定性未確認のため本講座では使わない}
\end{frame}
...
\begin{frame}{本日の進め方}
  ...
  \begin{alertblock}{各セッションの進め方}
    \footnotesize 地図（今どこにいるか）→ このセッションで伝えること → デモ → 理論とコードの対応表 → 復習パス → チェックポイント
  \end{alertblock}
\end{frame}

% ------------------------------------------------------------------
\begin{frame}[fragile]{参加のしかたと資料}
  \begin{block}{デモの3段階}
    \begin{itemize}\small
...
\begin{frame}{今日の地図}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      \vspace{0.1em}
      {\scriptsize S2・S3 は「実装」、S5 は「解析・教育」を担当}
    \end{column}
...
\begin{frame}{必要機材と安全ルール}
  ...
  \begin{alertblock}{飛行時の安全ルール}
    \begin{itemize}\small\setlength{\itemsep}{2pt}
      \item 実機の飛行は講師が行う。飛んでいる機体からは距離を保つ
