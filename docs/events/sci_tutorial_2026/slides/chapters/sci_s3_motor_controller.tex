...
% ------------------------------------------------------------------
\begin{frame}{開発サイクルの地図と今の位置（セッション3）}
  \begin{columns}[c]
    ...
    \begin{column}{0.47\textwidth}
      \small
      セッション2でセンサの値を読めるようになった。ここでは読んだ値を使って機体を動かす側（モータ・コントローラ）を作る
    \end{column}
  \end{columns}
\end{frame}

% ------------------------------------------------------------------
\begin{frame}{モータを回し、操縦し、それだけでは飛べないと知る}
  \begin{enumerate}
...
\begin{frame}[fragile]{ペアリングとチャンネル}
  \begin{block}{ペアリング}
    \footnotesize コントローラの M5 ボタン（LCD パネル）を押しながら電源ON $\to$ StampFly のボタンを \warn{3 秒以上}押し続け、両方のビープで離す（5 秒以上でシステムリセット）。次回以降は自動接続
  \end{block}

  \vspace{0.3em}

  \begin{alertblock}{チャンネル設定}
    \footnotesize 教室内で複数機体が同時に飛ぶと混信する。チャンネルは実習 1 (2/2) の初期設定で \code{param set wifi.channel N}（1/6/11、講師が指定）として保存済み。実習コードの中では変更しない。コントローラはペアリング時に機体のチャンネルへ自動追従する
  \end{alertblock}
...
\begin{frame}{ミキサ行列}
  ...
  % Web版のみ: T/R/P/Yスライダーで4モータのDutyを実際に動かして確認する
  % @web: widget=mixer height=480
  % @web: layout=replace
\end{frame}
...
\begin{frame}{プラントの定義}
  ...
  \begin{exampleblock}{次セッションへ}
    \footnotesize セッション4 の PID 制御器がこの間にフィードバックループを作り，出力を目標値に近づける。伝達関数とゲイン $K_p$ の設計がセッション4「フィードバック制御の基礎」の主題である
  \end{exampleblock}
\end{frame}
...
\begin{frame}{理論とコードの対応表\later}
  ...
  \begin{table}
    ...
    \begin{tabular}{>{\raggedright\arraybackslash}p{30mm}>{\raggedright\arraybackslash}p{40mm}>{\raggedright\arraybackslash}p{35mm}>{\raggedright\arraybackslash}p{26mm}}
      \hline
      \textbf{理論} & \textbf{コード} & \textbf{通信} & \textbf{トピック} \\
      \hline
      Duty比 $[0,1]$ & \api{ws::\allowbreak motor\_set\_duty(id,d)} & --- & \code{actuator\_\allowbreak motor} \\
      スティック指令 $T,R,P,Y$ & \api{ws::\allowbreak rc\_throttle()} 等 & \code{ControlPacket}（14B） & \code{command\_\allowbreak setpoint} \\
      ミキシング & \api{ws::\allowbreak motor\_mixer(T,R,P,Y)} & --- & --- \\
      \hline
    \end{tabular}
  \end{table}
\end{frame}
...
\begin{frame}{復習パス\later}
  \begin{table}
    \centering\footnotesize
    \setlength{\tabcolsep}{4pt}
    \begin{tabular}{>{\raggedright\arraybackslash}p{40mm}>{\raggedright\arraybackslash}p{35mm}>{\raggedright\arraybackslash}p{55mm}}
      \hline
      \textbf{コマンド} & \textbf{実習} & \textbf{文書} \\
      \hline
      \code{sf lesson switch sci2026:3} & 実習 3 モータ制御 & \code{firmware/\allowbreak vehicle/\allowbreak docs/\allowbreak hardware\_init.md} \\
      \code{sf lesson switch sci2026:4} & 実習 4 コントローラ入力 & \code{protocol/\allowbreak spec/}（ControlPacket） \\
      \code{sf flash controller} & --- & \code{docs/\allowbreak commands/\allowbreak sf-flasher.md} \\
      \hline
    \end{tabular}
  \end{table}
...
\begin{frame}{チェックポイント}
  ...
  \begin{block}{次のセッション}
    セッション4: フィードバック制御の基礎 --- PID による姿勢安定化（14:00 -- 15:00）
  \end{block}
\end{frame}
