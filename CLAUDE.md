...
| T3 | **矢印の接続・方向統一・ノード貫通禁止:** 矢印の始点/終点がブロック・加算点・分岐点に接続し、空白から発生/消滅していないか。同じ役割の矢印群の矢頭方向が統一されているか。**矢印が他のノードを貫通していないか**（2x2 グリッド等で下段ノードへの矢印が上段ノードを通過する問題。`|-` や `-|` の経路は座標を計算して中間ノードとの交差を検証すること — 画像レビューでは検出不可） | espnow\_dataflow Yaw矢印がThrottleボックスを貫通 |
