...
- **単独でビルド・実行可能**（vehicle全体のビルド不要）
  - 例外: **L1 組み込み型テンプレート**（`sf app new` の複製元、`11_app_controller` 等）は vehicle 本体と一緒にビルドされる（`SF_APP_DIR` で main コンポーネントに直接コンパイル）。単独ベンチの例題（09 / 10）と区別し、README 冒頭に「組み込み型（実機・SILS で飛ぶ）」か「単独ベンチ（飛ばない）」かを明記する。設計は [`docs/plans/sf-app-sils-plan.md`](../../../docs/plans/sf-app-sils-plan.md) §2
