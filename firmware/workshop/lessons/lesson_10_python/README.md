...
stable PID controller (from Lesson 6). You will use the Python SDK and `sf` CLI to send
high-level commands like takeoff, hover, and land.

このレッスンではC++ファームウェアからPythonベースの制御に移行する。ファームウェア側は
Lesson 6の安定したPID制御を実行する。Python SDKと`sf` CLIを使って離陸、ホバリング、
着陸などの高レベルコマンドを送信する。
...
Lesson 6. The focus is on the Python side, not the C++ side.

このレッスンでは `student.cpp` と `solution.cpp` はどちらもLesson 6のPID制御と同じ内容。
